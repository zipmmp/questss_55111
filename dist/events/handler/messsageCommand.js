...
export default class messageHandler extends baseDiscordEvent {
    ...
    async executeEvent(message) {
        if (!message.content || message.author.bot || !this.client.ready)
            return;
        ...
        if (!command.enabled)
            return replyMessage(i18n.t("commandDisabled"));
        ...
        if (!argsCheck)
            return replyMessage(i18n.t("badUsage", { usage: command.getUsage(prefix) }));
        if (cd) {
        ...
        if (message.guild) {
            ...
            if (!command.isAllowedGuild(guild.id))
                return replyMessage(i18n.t("notAllowedGuild"));
            const user = author;
